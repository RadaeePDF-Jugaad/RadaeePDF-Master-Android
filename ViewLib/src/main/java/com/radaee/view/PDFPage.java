...
public class PDFPage implements IPDFLayoutView.IVPage {
    ...
    protected PDFPage(Document doc, int pageno, int cw, int ch, Bitmap.Config bmp_format) {
        m_zoom_bmp = null;
        m_result = 0;
    ...
    protected final int GetX() {
        return VNPage.getX(m_vpage);
    }

    protected final void SetX(int x) {
        VNPage.setX(m_vpage, x);
    }

    protected final int GetY() {
        return VNPage.getY(m_vpage);
    }

    protected final int GetWidth() {
        return VNPage.getWidth(m_vpage);
    }

    protected final int GetHeight() {
        return VNPage.getHeight(m_vpage);
    }

    protected final float GetPDFX(int vx) {
        return VNPage.getPDFX(m_vpage, vx);
    }

    protected final float GetPDFY(int vy) {
        return VNPage.getPDFY(m_vpage, vy);
    }

    public final int GetPageNo() {
        return VNPage.getPageNo(m_vpage);
    }

    public final int GetVX(float pdfx) {
        return VNPage.getVX(m_vpage, pdfx);
    }

    public final int GetVY(float pdfy) {
        return VNPage.getVY(m_vpage, pdfy);
    }

    protected int LocVert(int y, int gap_half) {
        return VNPage.locVert(m_vpage, y, gap_half);
    }

    protected int LocHorz(int x, int gap_half) {
        return VNPage.lovHorz(m_vpage, x, gap_half);
    }
    ...
    protected void vClips(VNPage.VNPageListener callback, boolean clip) {
        VNPage.clips(m_vpage, callback, clip);
    }
    ...
    protected boolean vFinished() {
        return VNPage.finished(m_vpage);
    }

    protected void vRenderAsync(VNPage.VNPageListener callback, int vx, int vy, int vw, int vh) {
        VNPage.renderAsync(m_vpage, callback, vx, vy, vw, vh);
    }

    protected void vRenderSync(VNPage.VNPageListener callback, int vx, int vy, int vw, int vh) {
        VNPage.renderSync(m_vpage, callback, vx, vy, vw, vh);
    }
    ...
    protected void vCacheStart1(VNPage.VNPageListener callback) {
        VNPage.blkStart1(m_vpage, callback);
    }
    ...
    protected void vCacheEnd(VNPage.VNPageListener callback) {
        VNPage.blkEnd(m_vpage, callback);
    }

    protected void vDraw(VNPage.VNPageListener callback, BMP bmp, int vx, int vy) {
        m_result = VNPage.Draw(m_vpage, callback, bmp, vx, vy);
    }

    protected boolean vDrawStep1(VNPage.VNPageListener callback, Canvas canvas) {
        return VNPage.drawStep1(m_vpage, callback, canvas, m_result);
    }

    protected void vDrawStep2(BMP bmp) {
        VNPage.DrawStep2(m_vpage, bmp, m_result);
    }

    protected void vDrawEnd() {
        VNPage.resultDestroy(m_result);
        m_result = 0;
    }
    ...
    public final float ToPDFX(float x, float scrollx) {
        return VNPage.toPDFX(m_vpage, x, scrollx);
    }
    ...
    public final float ToPDFY(float y, float scrolly) {
        return VNPage.toPDFY(m_vpage, y, scrolly);
    }
    ...
    public final float ToDIBX(float x) {
        return VNPage.toDIBX(m_vpage, x);
    }
    ...
    public final float ToDIBY(float y) {
        return VNPage.toDIBY(m_vpage, y);
    }
