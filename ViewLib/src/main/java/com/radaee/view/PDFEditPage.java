...
public class PDFEditPage implements IPDFLayoutView.IVPage {
    ...
    private PDFEditBlock[] m_blks;
    private PDFEditBlock[] m_blks_zoom;

    public PDFEditPage(Document doc, int pageno) {
        m_doc = doc;
        m_pageno = pageno;
        m_blks = null;
        m_blks_zoom = null;
    ...
    protected final void gl_alloc() {
        ...
        int csize2 = PDFEditBlock.m_cell_size << 1;
        if (!m_curl && PDFEditBlock.m_cell_size > 0 && (width >= csize2 || height >= csize2)) {
            int xcnt = (width + PDFEditBlock.m_cell_size - 1) / PDFEditBlock.m_cell_size;
            int ycnt = (height + PDFEditBlock.m_cell_size - 1) / PDFEditBlock.m_cell_size;
            m_blks = new PDFEditBlock[xcnt * ycnt];
            int cury = 0;
            for (int yb = 0; yb < ycnt; yb++) {
                int curx = 0;
                int bheight = (yb < ycnt - 1) ? PDFEditBlock.m_cell_size : height - cury;
                for (int xb = 0; xb < xcnt; xb++) {
                    int bwidth = (xb < xcnt - 1) ? PDFEditBlock.m_cell_size : width - curx;
                    m_blks[yb * xcnt + xb] = new PDFEditBlock(m_doc, m_pageno, m_scale, curx, cury, bwidth, bheight, height);
                    curx += bwidth;
                }
                cury += bheight;
            }
        } else {
            m_blks = new PDFEditBlock[1];
            m_blks[0] = new PDFEditBlock(m_doc, m_pageno, m_scale, 0, 0, m_right - m_left, m_bottom - m_top, height);
        }
    }
    ...
    public final void gl_render(GL10 gl10, PDFEditThread thread) {
        thread.render_start(m_blks[0]);
    }

    public final void gl_draw_curl(GL10 gl10, PDFEditThread thread, int def_text, int shadow_text, int type, int x, int y) {
        if (!m_blks[0].gl_make_text()) thread.render_start(m_blks[0]);
        m_blks[0].gl_draw_curl(gl10, def_text, type, x, y, shadow_text);
    }

    public final void gl_draw(GL10 gl10, PDFEditThread thread, int def_text, int orgx, int orgy, int w, int h) {
        if (m_dirty) {
            m_dirty = false;
        ...
        if (bb > h) bb = h;
        if (br > bl && bb > bt)
            PDFEditBlock.drawQuadColor(gl10, def_text, bl << 16, bt << 16, br << 16, bt << 16, bl << 16, bb << 16, br << 16, bb << 16, 1, 1, 1);
        int left = m_left - orgx;
        int top = m_top - orgy;
        if (m_blks_zoom != null)//draw zoom cache first.
        {
            PDFEditBlock glb = m_blks_zoom[m_blks_zoom.length - 1];
            int srcw = glb.GetRight();
            int srch = glb.GetBottom();
            int dstw = m_right - m_left;
            int dsth = m_bottom - m_top;
            for (PDFEditBlock pdfEditBlock : m_blks_zoom) {
                glb = pdfEditBlock;
        ...
        boolean all_ok = true;
        bl = -left - PDFEditBlock.m_cell_size;
        bt = -top - PDFEditBlock.m_cell_size;
        br = w - left + PDFEditBlock.m_cell_size;
        bb = h - top + PDFEditBlock.m_cell_size;
        for (PDFEditBlock glb : m_blks) {
     ...
     * @param h
     */
    public final void gl_draw2(GL10 gl10, PDFEditThread thread, int def_text, int orgx, int orgy, int w, int h) {
        if (m_dirty) {
            m_dirty = false;
            gl_zoom_start(gl10, thread);
            gl_alloc();
        ...
        br = w - left;
        bb = h - top;
        int bl1 = -left - w - PDFEditBlock.m_cell_size;
        int bt1 = -top - PDFEditBlock.m_cell_size;
        int br1 = w - left + w + PDFEditBlock.m_cell_size;
        int bb1 = h - top + PDFEditBlock.m_cell_size;
        for (PDFEditBlock glb : m_blks) {
    ...
    public final void gl_sync(int orgx, int orgy, int w, int h) {
        ...
        int top = m_top - orgy;
        if (m_blks == null) return;//then draw the blocks.
        int bl = -left - PDFEditBlock.m_cell_size;
        int bt = -top - PDFEditBlock.m_cell_size;
        int br = w - left + PDFEditBlock.m_cell_size;
        int bb = h - top + PDFEditBlock.m_cell_size;
        for (PDFEditBlock glb : m_blks) {
            if (glb.isCross(bl, bt, br, bb)) {
    ...
    public final void gl_end(GL10 gl10, PDFEditThread thread) {
        if (m_blks == null) return;
        for (int cur = 0; cur < m_blks.length; cur++) {
            PDFEditBlock glb = m_blks[cur];
            if (thread.render_end(gl10, glb)) m_blks[cur] = new PDFEditBlock(glb, m_doc);
        }
    }

    public final void gl_end_zoom(GL10 gl10, PDFEditThread thread) {
        if (m_blks_zoom == null) return;
        for (PDFEditBlock pdfEditBlock : m_blks_zoom) thread.render_end(gl10, pdfEditBlock);
        m_blks_zoom = null;
    }

    protected final void gl_zoom_start(GL10 gl10, PDFEditThread thread) {
        if (m_blks_zoom != null) {
            if (m_blks != null) {
                for (PDFEditBlock glb : m_blks) {
    ...
    public Bitmap Reflow(int w, float scale, boolean render_images) {
        if (size_limit <= 0) size_limit = PDFEditBlock.m_cell_size * PDFEditBlock.m_cell_size * 4;
