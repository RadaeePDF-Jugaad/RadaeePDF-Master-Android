...
public class PDFGLLayoutDual extends PDFGLLayout {
    ...
    public PDFGLLayoutDual(Context context, int align, int scale_mode, boolean rtol, boolean[] horz_dual, boolean[] vert_dual) {
        super(context);
    ...
    protected void gl_flush_range(GL10 gl10)
    {
        ...
        if(m_rtol)
        {
            pageno1 = range_p1(-m_vw - PDFGLBlock.m_cell_size, -PDFGLBlock.m_cell_size);
            pageno2 = range_p0(m_vw * 2 + PDFGLBlock.m_cell_size, m_vh + PDFGLBlock.m_cell_size);
        }
        else
        {
            pageno1 = range_p0(-m_vw - PDFGLBlock.m_cell_size, -PDFGLBlock.m_cell_size);
            pageno2 = range_p1(m_vw * 2 + PDFGLBlock.m_cell_size, m_vh + PDFGLBlock.m_cell_size);
        }
    ...
    public int vGetPage(int vx, int vy) {
        ...
        while (pr >= pl) {
            ...
            if (vx < pmid.left - hg)
                ...
                PDFGLPage  page = m_pages[pmid.page_left];
    ...
    public void vSetPos(int vx, int vy, PDFEditLayout.PDFPos pos) {
        ...
        PDFGLPage gpage = m_pages[pos.pageno];
    ...
    public void vGotoPage(int pageno) {
        if (m_pages == null || m_doc == null || m_vw <= 0 || m_vh <= 0 || pageno < 0 || pageno >= m_page_cnt)
            return;
        gl_abort_scroll();
        PDFGLPage  gpage = m_pages[pageno];
