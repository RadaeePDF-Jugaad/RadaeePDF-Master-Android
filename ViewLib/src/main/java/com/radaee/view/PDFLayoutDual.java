...
public class PDFLayoutDual extends PDFLayout {
    ...
    public PDFLayoutDual(Context context) {
        super(context);
        Global.g_auto_scale = context.getResources().getBoolean(R.bool.reader_fit_different_page_size);
        ...
        if (!m_rtol)//ltor
        {
            while (left <= right) {
                ...
                if (vx < pg1.left) {
                    ...
                    PDFPage vpage = m_pages[pg1.page_left];
    ...
    public void vGotoPage(int pageno) {
        ...
        if (Global.g_auto_scale && vGetScale() > vGetMinScale())
    ...
    public void vScrolltoPage(int pageno) {
        if (m_pages == null || m_doc == null || m_w <= 0 || m_h <= 0 || pageno < 0 || pageno >= m_cells.length)
            return;
        if (Global.g_auto_scale && vGetScale() > vGetMinScale())
            vZoomSet(vGetWidth() / 2, vGetHeight() / 2, vGetPos(0, 0), 1);
        vScrollAbort();
    ...
    public void vZoomSet(int vx, int vy, PDFPos pos, float zoom) {
        ...
        if (Global.g_auto_scale) {
            ...
            if (Global.g_view_mode == 6 && m_w > m_h) {
