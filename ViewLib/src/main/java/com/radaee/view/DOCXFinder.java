...
import com.radaee.docx.Page.Finder;

/**
 * Inner class.<br/>
 * developer should not use this.
 */
public class DOCXFinder {
    ...
    private void find_draw(Canvas canvas, DOCXGLPage page, int index, Paint paint, int scrollx, int scrolly) {
        ...
        while (ichar < ichar_end) {
            ...
            if (rect_word[1] == rect[1] && rect_word[3] == rect[3] &&
                ...
                rect_draw[1] = page.GetVY(rect_word[1]) - scrolly;
                rect_draw[2] = page.GetVX(rect_word[2]) - scrollx;
                rect_draw[3] = page.GetVY(rect_word[3]) - scrolly;
    ...
    protected void find_draw(Canvas canvas, DOCXGLPage page, int scrollx, int scrolly)//draw current found
    {
        if (!is_cancel) {
            eve_wait();
            is_cancel = true;
