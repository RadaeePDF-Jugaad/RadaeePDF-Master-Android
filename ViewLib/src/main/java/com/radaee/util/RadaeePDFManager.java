...
public class RadaeePDFManager implements RadaeePluginCallback.PDFReaderListener {
    ...
    public static boolean sHideToolButton = false;
    ...
    public void show(Context context, String url, String password, boolean readOnlyMode, boolean automaticSave, int gotoPage, String bmpFormat, String author) {
        if (!TextUtils.isEmpty(url)) {
            ...
            Intent intent = new Intent(context, PDFGLViewAct.class);
    ...
    public void didShowReader() {
        ...
        if (mIconsBgColor != -1)
            RadaeePluginCallback.getInstance().onSetIconsBGColor(mIconsBgColor);
        if (mTitleBgColor != -1)
            RadaeePluginCallback.getInstance().onSetToolbarBGColor(mTitleBgColor);
