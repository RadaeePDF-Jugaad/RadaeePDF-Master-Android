...
public class PageContent
{
	...
	static private native int drawText3(long content, String text, int align, float width, int max_lines);
	...
	static private native void tagBlockStart(long content, long block);
	static private native void tagBlockEnd(long content);
	 ...
	 */
	final public int DrawText(String text, int align, float width)
	{
		return drawText2(hand, text, align, width);
	}
	/**
	 * show text with special width
	 * @param text text to show.
	 ...
	 * @return 2 element as [count of unicodes have drawn, count of line have drawn].
	 */
	final public int[] DrawText(String text, int align, float width, int max_lines)
	{
		int val = drawText3(hand, text, align, width, max_lines);
		int[] ret = new int[2];
		ret[0] = val & ((1<<20) - 1);
		ret[1] = val >> 20;
		return ret;
	...
	/**
	 * start a tag, this function shall pair with TagBlockEnd().
	 * @param blk block tag return from Page.NewTagBlock();
	 */
	final public void TagBlockStart(Page.TagBlock blk)
	{
		if(blk == null) return;
		tagBlockStart(hand, blk.m_hand);
	}

	/**
	 * end a tag, this function shall pair with TagBlockStart();
	 */
	final public void TagBlockEnd()
	{
		tagBlockEnd(hand);
	}
    @Override
