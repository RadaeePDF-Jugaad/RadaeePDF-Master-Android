...
public class OFDGLViewAct extends Activity implements IOFDLayoutView.OFDLayoutListener {
    ...
    private OFDGLLayoutView m_view = null;
    private OFDViewController m_controller = null;
    ...
    private void ProcessOpenResult(int ret) {
        switch (ret) {
            ...
            case -3://damaged or invalid format
                onFail("Open Failed: Damaged or Invalid OFD file");
                break;
            ...
            case 0://succeeded, and continue
                ...
                task.execute();
                //m_view.OFDOpen(m_doc, this);
                //m_controller = new OFDViewController(m_layout, m_view);
                break;
    ...
    class OpenTask extends AsyncTask<Void, Integer, Integer> {
        ...
        protected void onPreExecute() {
            ...
            runable = new Runnable() {
                public void run() {
                    dlg = ProgressDialog.show(OFDGLViewAct.this, "Please wait", "Loading OFD file...", true);
                }
            };
        ...
        protected void onPostExecute(Integer integer) {
            m_view.OFDOpen(m_doc, OFDGLViewAct.this);
            m_controller = new OFDViewController(m_layout, m_view);
            if (dlg != null)
                dlg.dismiss();
            else
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Global.Init(this);
        m_layout = (RelativeLayout) LayoutInflater.from(this).inflate(R.layout.ofd_gllayout, null);
        m_view = (OFDGLLayoutView) m_layout.findViewById(R.id.ofd_glview);
        ...
        if (ms_tran_doc != null) {
    		...
    		*/
            ...
            if (docx_http != null && docx_http != "") {
                ...
                new Thread(){
                    ...
                    public void run() {
                        ...
                        m_layout.post(new Runnable() {
                            ...
                            public void run() {
                                m_pkg = new Package();
                                int ret = m_pkg.OpenStream(m_http_stream, docx_pswd);
                ...
                m_pkg = new Package();
                int ret = m_pkg.OpenStream(m_asset_stream, docx_pswd);
                ProcessOpenResult(ret);
            } else if (docx_path != null && docx_path != "") {
                m_pkg = new Package();
                int ret = m_pkg.Open(docx_path, docx_pswd);
    ...
    protected void onDestroy() {
        final OFDViewController vctrl = m_controller;
        final Document doc = m_doc;
        final OFDGLLayoutView view = m_view;
        ...
        new Thread(){
            ...
            public void run() {
                ...
                Global.RemoveTmp();
                synchronized (OFDGLViewAct.this) { OFDGLViewAct.this.notify(); }
            }
        }.start();
        synchronized (this)
        {
        ...
        View.OnClickListener TextSelectionListener = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (v.getId() == R.id.icon_copy) {
                    Toast.makeText(OFDGLViewAct.this, getString(R.string.text_copy_hint) + text, Toast.LENGTH_SHORT).show();
    ...
    public void OnOFDOpenURI(String uri) {
        try {
            ...
            Toast.makeText(OFDGLViewAct.this, "todo: open url:" + uri, Toast.LENGTH_SHORT).show();
        }
    }
