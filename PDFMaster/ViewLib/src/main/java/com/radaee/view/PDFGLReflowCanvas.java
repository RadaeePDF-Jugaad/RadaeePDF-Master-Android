...
public class PDFGLReflowCanvas
{
    private int m_w;
    private int m_h;
    private int m_gap;
    private int m_cell_h;
    private PDFGLReflowBlock[] m_blks;
    private Page m_page;
    public PDFGLReflowCanvas(Document doc, int pageno, float scale, int w, int cell_h, int gap)
    {
        ...
        m_blks = new PDFGLReflowBlock[blk_cnt];
        int blk_cur = 0;
        int y = 0;
        for(blk_cur = 0; blk_cur < blk_cnt - 1; blk_cur++)
        {
            m_blks[blk_cur] = new PDFGLReflowBlock(m_page, pageno, y, m_w, m_cell_h, m_gap);
            y += m_cell_h;
        }
        m_blks[blk_cur] = new PDFGLReflowBlock(m_page, pageno, y, m_w, m_h - y, m_gap);//last block
    }
    public void gl_draw(GL10 gl10, PDFGLThread thread, int def_text, int vy, int vh)
    {
        if(m_blks == null) return;
        int blk_cur = 0;
        for(blk_cur = 0; blk_cur < m_blks.length; blk_cur++)
        {
            PDFGLReflowBlock blk = m_blks[blk_cur];
    ...
    public void gl_close(GL10 gl10, PDFGLThread thread)
    {
        int blk_cur = 0;
        for(blk_cur = 0; blk_cur < m_blks.length; blk_cur++) {
            PDFGLReflowBlock blk = m_blks[blk_cur];
