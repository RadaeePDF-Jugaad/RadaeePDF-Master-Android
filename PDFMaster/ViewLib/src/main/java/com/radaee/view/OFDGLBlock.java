...
public class OFDGLBlock {
    ...
    protected OFDGLBlock(OFDGLBlock src, Document doc) {
        m_doc = doc;
    ...
    protected OFDGLBlock(Document doc, int pageno, float scale, int x, int y, int w, int h) {
        m_doc = doc;
        m_pageno = pageno;
        m_scale = scale;
        m_x = x;
    ...
    protected final void bk_render() {
        ...
        int sw = (int) (m_page.GetWidth() * m_scale);
        int sh = (int) (m_page.GetHeight() * m_scale);
        if (sw <= m_w && sh <= m_h)
            m_page.Render(dib, (sw - m_w) >> 1, (sh - m_h) >> 1, m_scale * 25.4f);
        else
            m_page.Render(dib, m_x, m_y, m_scale * 25.4f);
        m_dib_tmp = null;
        if (m_status == 1) {
            m_dib = dib;
            m_status = 2;
    ...
    protected final void gl_draw(GL10 gl10, int def_text, int left, int top, int right, int bottom) {
        ...
        if (m_vect == null)
            m_vect = create_buf(new int[]{left << 16, top << 16, right << 16, top << 16, left << 16, bottom << 16, right << 16, bottom << 16});
        else
        {
            m_vect.position(0);
            m_vect.put(left << 16);
