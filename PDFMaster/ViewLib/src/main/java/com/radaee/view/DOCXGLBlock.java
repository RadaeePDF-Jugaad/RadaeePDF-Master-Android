...
public class DOCXGLBlock {
    ...
    protected DOCXGLBlock(DOCXGLBlock src, Document doc) {
        m_doc = doc;
    ...
    protected DOCXGLBlock(Document doc, int pageno, float scale, int x, int y, int w, int h) {
        m_doc = doc;
        m_pageno = pageno;
        m_scale = scale;
        m_x = x;
    ...
    protected final void bk_render() {
        ...
        if (sw <= m_w && sh <= m_h)
            m_page.Render(dib, m_scale, (sw - m_w) >> 1, (sh - m_h) >> 1);
        else
            m_page.Render(dib, m_scale, m_x, m_y);
        m_dib_tmp = null;
        if (m_status == 1) {
            m_dib = dib;
            m_status = 2;
