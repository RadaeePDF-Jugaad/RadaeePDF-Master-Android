...
public class PDFEditLayoutDual extends PDFEditLayout {
    ...
    public PDFEditLayoutDual(Context context, int align, int scale_mode, boolean rtol, boolean[] horz_dual, boolean[] vert_dual) {
        super(context);
    ...
    protected void gl_flush_range(GL10 gl10) {
        ...
        if(m_rtol)
        {
            pageno1 = range_p1(-m_vw - PDFEditBlock.m_cell_size, -PDFEditBlock.m_cell_size);
            pageno2 = range_p0(m_vw * 2 + PDFEditBlock.m_cell_size, m_vh + PDFEditBlock.m_cell_size);
        }
        else
        {
            pageno1 = range_p0(-m_vw - PDFEditBlock.m_cell_size, -PDFEditBlock.m_cell_size);
            pageno2 = range_p1(m_vw * 2 + PDFEditBlock.m_cell_size, m_vh + PDFEditBlock.m_cell_size);
        }
        if (pageno1 >= 0 && pageno2 >= 0) {
            ...
            if (m_pageno1 < pageno1) {
                ...
                while (start < end) {
                    PDFEditPage vpage = m_pages[start];
    ...
    public int vGetPage(int vx, int vy) {
        ...
        while (pr >= pl) {
            ...
            if (vx < pmid.left - hg)
                ...
                PDFEditPage page = m_pages[pmid.page_left];
    ...
    private static class LayoutRec {
        float maxw = 0;
        float maxh = 0;
    ...
    private void layout_ltor(float scale, boolean zoom, boolean[] para) {
        ...
        if (m_lrec == null) {
            ...
            while (pcur < m_page_cnt) {
                ...
                if (m_lrec.maxw < cw) m_lrec.maxw = cw;
                if (m_lrec.maxh < ch) m_lrec.maxh = ch;
                float scalew = (m_vw - m_page_gap) / cw;
                float scaleh = (m_vh - m_page_gap) / ch;
                if (scalew > scaleh) scalew = scaleh;
                cw *= scalew;
                ch *= scalew;
                if (m_lrec.minscalew > (int) cw) m_lrec.minscalew = (int) cw;
                if (m_lrec.minscaleh > (int) ch) m_lrec.minscaleh = (int) ch;
                m_lrec.ccnt++;
            }
        }
        boolean changed = (m_cells == null || m_cells.length != m_lrec.ccnt);
        if (changed) m_cells = new PDFCell[m_lrec.ccnt];
        m_scale_min = (float) (m_vw - m_page_gap) / m_lrec.maxw;
        float scalew;
        float scaleh = (float) (m_vh - m_page_gap) / m_lrec.maxh;
        ...
        m_layh = 0;
        pcur = 0;
        for (int ccur = 0; ccur < m_lrec.ccnt; ccur++) {
            float cw = m_doc.GetPageWidth(pcur);
            float ch = m_doc.GetPageHeight(pcur);
            ...
            switch (m_scale_mode) {
                case SCALE_SAME_WIDTH:
                    scalew = m_lrec.minscalew / cw;
                    cell.scale = scalew / m_scale_min;
                    break;
                case SCALE_SAME_HEIGHT:
                    scaleh = m_lrec.minscaleh / ch;
        ...
        if (cell1 < cell2) {
            ...
            if (endx + m_vw > cell.right) {
                if (endx + (m_vw >> 1) > cell.right) {
                    ...
                    do_scroll(x, y, m_cells[cell2].right - m_vw - x, endy - y);
            } else if (endx < m_cells[cell2].left) {
                if (endx + (m_vw >> 1) < m_cells[cell2].left) {
                    cell2--;
                    if (cell2 < 0)
                        do_scroll(x, y, m_cells[0].left - x, endy - y);
                    else
                        do_scroll(x, y, m_cells[cell2].left - x, endy - y);
                } else
                    do_scroll(x, y, m_cells[cell2].left - x, endy - y);
    ...
    public void vGotoPage(int pageno) {
        if (m_pages == null || m_doc == null || m_vw <= 0 || m_vh <= 0 || pageno < 0 || pageno >= m_page_cnt)
            return;
        gl_abort_scroll();
        PDFEditPage gpage = m_pages[pageno];
