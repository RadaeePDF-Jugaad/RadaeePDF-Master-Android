...
abstract public class PDFEditLayout {
    ...
    protected PDFEditPage[] m_pages;
    protected int m_page_gap;
    protected int m_page_cnt;
    protected Document m_doc;
    protected PDFEditFinder m_finder;
    protected EditListener m_listener;
    private final Handler m_hand_gl = new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    m_listener.OnBlockRendered(((PDFEditBlock) msg.obj).GetPageNo());
                    msg.obj = null;
                    break;
    ...
    protected PDFEditThread m_thread;
    ...
    protected PDFEditLayout(Context context) {
        m_ctx = context;
        ...
        if (PDFEditBlock.m_cell_size <= 0) {
            WindowManager wm = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
            DisplayMetrics dm = new DisplayMetrics();
            wm.getDefaultDisplay().getMetrics(dm);
            PDFEditBlock.m_cell_size = dm.widthPixels;
            if (PDFEditBlock.m_cell_size > dm.heightPixels)
                PDFEditBlock.m_cell_size = dm.heightPixels;
            //if (PDFEditBlock.m_cell_size > 1024) PDFEditBlock.m_cell_size = 1024;
            //else PDFEditBlock.m_cell_size = 512;
        }
    }

    public void vOpen(Document doc, EditListener listener, int page_gap) {
        m_doc = doc;
        m_page_gap = page_gap;
        m_finder = new PDFEditFinder();
        m_listener = listener;
        m_thread = new PDFEditThread();
        m_thread.set_handler(m_hand_gl);
        m_thread.start();
        m_page_cnt = m_doc.GetPageCount();
        m_pages = new PDFEditPage[m_page_cnt];
        for (int pcur = 0; pcur < m_page_cnt; pcur++) {
            m_pages[pcur] = new PDFEditPage(m_doc, pcur);
        }
    }
    ...
    protected void gl_flush_range(GL10 gl10) {
        ...
        int pageno1 = vGetPage(-PDFEditBlock.m_cell_size, -PDFEditBlock.m_cell_size);
        int pageno2 = vGetPage(m_vw + PDFEditBlock.m_cell_size, m_vh + PDFEditBlock.m_cell_size);
    ...
    public void gl_fill_color(GL10 gl10, int left, int top, int right, int bottom, float r, float g, float b) {
        PDFEditBlock.drawQuadColor(gl10, m_def_text, left << 16, top << 16, right << 16, top << 16, left << 16, bottom << 16, right << 16, bottom << 16, r, g, b);
    }
    ...
    public final PDFEditPage vGetPage(int pageno) {
    ...
    public PDFPos vGetPos(int vx, int vy) {
        ...
        PDFEditPage gpage = m_pages[pgno];
     ...
     */
    public void gl_render(PDFEditPage page) {
    ...
    public final void vClear(GL10 gl10, int color) {
        ...
        PDFEditBlock.drawQuadColor(gl10, 0, 0, 0, m_vw << 16, 0, 0, m_vh << 16, m_vw << 16, m_vh << 16,
