...
abstract public class OFDGLLayout {
    ...
    protected OFDGLPage m_pages[];
    protected int m_page_gap;
    protected int m_page_cnt;
    protected Document m_doc;
    protected OFDFinder m_finder;
    ...
    private Handler m_hand_gl = new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    m_listener.OnBlockRendered(((OFDGLBlock) msg.obj).GetPageNo());
                    msg.obj = null;
                    break;
    ...
    protected OFDGLThread m_thread;
    ...
    protected OFDGLLayout(Context context) {
        m_ctx = context;
        ...
        if (OFDGLBlock.m_cell_size <= 0) {
            WindowManager wm = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
            DisplayMetrics dm = new DisplayMetrics();
            wm.getDefaultDisplay().getMetrics(dm);
            int psize = dm.widthPixels;
            if (dm.heightPixels < psize) psize = dm.heightPixels;
            //if (psize < 1024) OFDGLBlock.m_cell_size = 512;
            //else OFDGLBlock.m_cell_size = 1024;
        }
    }
    ...
    public void vOpen(Document doc, GLListener listener, int page_gap) {
        ...
        m_page_gap = page_gap;
        m_listener = listener;
        m_finder = new OFDFinder();
        m_thread = new OFDGLThread();
        m_thread.set_handler(m_hand_gl);
        m_thread.start();
        m_page_cnt = m_doc.GetPageCount();
        m_pages = new OFDGLPage[m_page_cnt];
        for (int pcur = 0; pcur < m_page_cnt; pcur++) {
            m_pages[pcur] = new OFDGLPage(m_doc, pcur);
        }
    }
    ...
    protected void gl_flush_range(GL10 gl10) {
        ...
        int pageno1 = vGetPage(-OFDGLBlock.m_cell_size, -OFDGLBlock.m_cell_size);
        int pageno2 = vGetPage(m_vw + OFDGLBlock.m_cell_size, m_vh + OFDGLBlock.m_cell_size);
    ...
    static public class OFDPos {
        public float x = 0;
        public float y = 0;
        public int pageno = 0;
    }

    public final OFDGLPage vGetPage(int pageno) {
    ...
    public void vSetPos(int vx, int vy, OFDPos pos) {
        if (pos == null) return;
        OFDGLPage gpage = m_pages[pos.pageno];
    ...
    public OFDPos vGetPos(int vx, int vy) {
        int pgno = vGetPage(vx, vy);
        if (pgno < 0 || pgno >= m_page_cnt) return null;
        OFDGLPage gpage = m_pages[pgno];
        OFDPos pos = new OFDPos();
        pos.pageno = pgno;
        pos.x = gpage.GetOFDX(vGetX() + vx);
        pos.y = gpage.GetOFDY(vGetY() + vy);
        return pos;
    }
    ...
    public void gl_resize(int cx, int cy) {
        ...
        gl_abort_scroll();
        OFDPos pos = vGetPos(m_vw >> 1, m_vh >> 1);
        m_vw = cx;
        m_vh = cy;
     ...
     */
    public void gl_render(OFDGLPage page) {
