...
public class UIEditMenu {
    ...
    public UIEditMenu(RelativeLayout parent) {
    ...
    public boolean show(EditNode edit, float[] annot_rect, IMemnuCallback callback) {
        m_edit = edit;
        m_callback = callback;
        int atype = edit.GetType();
        boolean is_show = true;
        m_has_edit = (atype == 1);
        m_has_remove = (atype != 0);
        m_has_property = (atype == 3 || atype == 4);

        int btnsCount = 0;
        m_view.findViewById(R.id.btn_annot_perform).setVisibility(View.GONE);
    ...
    private float calculateY(float[] annot_rect) {
        ...
        if (y < 0)
            y = annot_rect[3] + m_view.getContext().getResources().getDimensionPixelOffset(R.dimen.annot_menu_bar_margin);
        return y;
    }
}
