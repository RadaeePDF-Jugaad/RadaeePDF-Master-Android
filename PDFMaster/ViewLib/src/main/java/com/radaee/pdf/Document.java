...
public class Document
{
	...
	static public class ImportContext
	{
		 ...
		 * @param srcno 0 based page NO. from source Document that passed to ImportStart.
		 * @param dstno 0 based page NO. to insert in this document object.
		 * @param left srcno Page display area in dstno Page.
		 * @param top srcno Page display area in dstno Page.
		 * @param right srcno Page display area in dstno Page.
		 * @param bottom srcno Page display area in dstno Page.
		 * @return true or false.
		 */
		public boolean ImportPage2Page( int srcno, int dstno, float left, float top, float right, float bottom)
		{
			if (hand == 0) return false;
			return Document.importPage2Page(m_doc.hand_val, hand, srcno, dstno, left, top, right, bottom);
		}
		/**
	...
	private static native long createForStream( RDStream stream );
	...
	private static native long openStreamWithCert( RDStream stream, String cert_file, String password );
	...
	private static native boolean delEF(long hand, int index);
	private static native boolean newEF(long hand, String path);
	...
	private static native String exportXFDF( long hand, String href );
	private static native boolean importXFDF( long hand, String xfdf );
	private static native boolean exportToDocx(long doc, int mode, int pg0, int pg1, String user_name, String path, String mod_path, String tmp_path);	private static native void close( long hand );
	...
	private static native boolean optimizeAs(long doc, String dst, byte[] options, float img_dpi);
	...
	private static native boolean importPage2Page( long hand, long ctx, int srcno, int dstno, float left, float top, float right, float bottom );
	...
	private static native long newImage( long hand, Bitmap bmp, boolean has_alpha, boolean interpolate );
	private static native long newImage2( long hand, Bitmap bmp, int matte, boolean interpolate );
	private static native long newImageJPEG( long hand, String path, boolean interpolate );
    private static native long newImageJPEGByArray( long hand, byte[] data, int length, boolean interpolate );
	private static native long newImageJPX( long hand, String path, boolean interpolate );
	private static native long newTagGroup(long doc, long parent, String tag);
	...
	private static native void setGenPDFA(boolean val);
	private static native int verifyPDFA(long doc);
	private static native long htmExpStart(long doc, String path);
	private static native boolean htmExpPage(long doc, long exp, int pageno);
	private static native void htmExpEnd(long doc, long exp);
	private static native void htmExpBinEnd(long doc, long exp);
	public class HtmlExportor
	{
		private HtmlExportor(long hand)
		{
			m_hand = hand;
		}
		private long m_hand;
		...
		/**
		 * export page from PDF file, it can only export 3 pages in demo project.<br/>
		 * this function require premium license.
		 * @param pageno
		 * @return
		 */
		public boolean ExportPage(int pageno)
		{
			return htmExpPage(hand_val, m_hand, pageno);
		}

		/**
		 * save to html and then free all native objects.
		 * @param server
		 */
		public void ExportEnd(boolean server)
		{
			if(server) htmExpBinEnd(hand_val, m_hand);
			else htmExpEnd(hand_val, m_hand);
		}
	}
	 ...
	 * @return 0 or less than 0 means failed, same as Open.
	 */
	public int CreateForStream( RDStream stream ) throws Exception
	{
		if( hand_val == 0 )
		{
			int ret = 0;
	...
	public int OpenStream( RDStream stream, String cert_file, String password )
	{
		if( hand_val == 0 )
		{
			int ret = 0;
	 ...
	 * @return
	 */
	public boolean DelEmbedFile(int index)
	{
		return delEF(hand_val, index);
	}
	 ...
	 * @return
	 */
	public boolean NewEmbedFile(String path)
	{
		return newEF(hand_val, path);
	}
	/**
	...
	/**
	 * Export XFDF format to string(xml).
	 * @param href reference to source PDF file, it can be null.
	 * @return a xml string with encoding header, defined by XFDF format.
	 */
	public String ExportXFDF(String href)
	{
		return exportXFDF( hand_val, href );
	}
	/**
	 * import XFDF format data from string.
	 * @param xfdf XFDF format string, that mean a xml string with encoding header.
	 * @return true or false.
	 */
	public boolean ImportXFDF(String xfdf)
	{
		return importXFDF( hand_val, xfdf );
	}
	/**
	 * export PDF file to docx(Miscrosoft Word) file.<br/>
	 * if no license actived, only first 3 pages can be exported.<br/>
	 * this function invoking will very slow, depends on PDF source file.
	 * @param mode 1: reflow mode, others: page mode.
	 * @param pg0 start page NO. start from 0.
	 * @param pg1 end page NO. pg1 must greater that pg0.
	 * @param user_name user name that write to docx file.
	 * @param path path of docx file, example: "/sdcard/test.docx".
	 * @param mod_path path of module file, example:"/sdcard/mod.dat"
	 * @param tmp_path template file to save some data, example: "/sdcard/cache/temp.dat"
	 * @return true or false.
	 */
	public boolean ExportToDocx(int mode, int pg0, int pg1, String user_name, String path, String mod_path, String tmp_path)
	{
		return exportToDocx(hand_val, mode, pg0, pg1, user_name, path, mod_path, tmp_path);
	}
	/**
	 ...
	 * this method require premium license.e
	 ...
	 */
	public DocImage NewImage( Bitmap bmp, boolean has_alpha, boolean interpolate )
	{
		long ret = newImage(hand_val, bmp, has_alpha, interpolate);
		if( ret != 0 )
		{
	...
	/**
	 * create an image from Bitmap object with alpha channel, and matte entry.
	 * @param bmp
	 * @param matte matte entry value.
	 * @param interpolate
	 * @return DocImage object or null.
	 */
	public DocImage NewImage( Bitmap bmp, int matte, boolean interpolate )
	{
		long ret = newImage2(hand_val, bmp, matte, interpolate);
		if( ret != 0 )
		{
			DocImage img = new DocImage();
			img.hand = ret;
	 ...
	 * @return DocImage object or null.
	 */
	public DocImage NewImageJPEG( String path, boolean interpolate )
	{
		long ret = newImageJPEG(hand_val, path, interpolate);
		if( ret != 0 )
		{
			DocImage img = new DocImage();
			img.hand = ret;
     ...
     * @return DocImage object or null.
     */
    public DocImage NewImageByMem( byte[] data, int len, boolean interpolate )
    {
        long ret = newImageJPEGByArray(hand_val, data, len, interpolate);
        if( ret != 0 )
        {
            DocImage img = new DocImage();
            img.hand = ret;
	 ...
	 * @return DocImage object or null.
	 */
	public DocImage NewImageJPX( String path, boolean interpolate )
	{
		long ret = newImageJPX(hand_val, path, interpolate);
		if( ret != 0 )
		{
			DocImage img = new DocImage();
			img.hand = ret;
	 ...
	 */
    public TagGroup NewTagGroup(TagGroup parent, String tag)
	{
		long hand = 0;
		if(parent != null)
			hand = newTagGroup(hand_val, parent.m_hand, tag);
		else
			hand = newTagGroup(hand_val, 0, tag);
		if(hand == 0) return null;
		return new TagGroup(hand);
	}
	/**
	 * change page rect.<br/>
	 * a premium license is required for this method.
	...
	/**
	 * set flag to generate PDFA header when Document.Save(), Document.SaveAs() or Document.EncryptAs() invoking.
	 * @param gen
	 */
	public void SetGenPDFA(boolean gen)
	{
		setGenPDFA(gen);
	}
	/**
	 * verify is PDF file a PDFA standard file?<br/>
	 * this method need premium license.
	 * @return error NO, 5 elements indicate chapter NO of "ISO_19005-2-2011", and return null if invalid parameter or no premium license actived.<br/>
	 * for example: [6,2,2,0,0] indicate to chapter 6.2.2.<br/>
	 * if all zero, means success.
	 */
	public byte[] VerifyPDFA()
	{
		int iret = verifyPDFA(hand_val);
		if(iret == -1) return null;
		byte[] bret = new byte[5];
		bret[0] = (byte)(iret >> 28);
		bret[1] = (byte)((iret >> 20) & 255);
		bret[2] = (byte)((iret >> 12) & 255);
		bret[3] = (byte)((iret >> 6) & 63);
		bret[4] = (byte)(iret & 63);
		return bret;
	}
	 ...
	 * @return
	 */
	public HtmlExportor NewHtmlExportor(String path)
	{
		long hand = htmExpStart(hand_val, path);
		if(hand == 0) return null;
		return new HtmlExportor(hand);
	}
